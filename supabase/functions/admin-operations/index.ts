...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === 'get-all-customers') {
      ...
      if (search) {
        usersQuery = usersQuery.or(`email.ilike.%${search}%,full_name.ilike.%${search}%,phone.ilike.%${search}%`);
      }
      ...
      const { data: loyaltyData, error: loyaltyError } = await supabase
        .from('loyalty_points')
        .select('user_id, total_points, lifetime_points, tier')
        .in('user_id', userIds);
      ...
      const data = (users || []).map(user => {
        const loyaltyPoints = loyaltyMap.get(user.id) || {
          total_points: 0,
          lifetime_points: 0,
          tier: 'Green Member'
        };
